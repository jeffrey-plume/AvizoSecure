...
class MainWindow(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        self.export_results_action.triggered.connect(self.export_results_to_csv)
        file_menu.addAction(self.export_results_action)
        self.export_results_action.setDisabled(True)
        
        # Add Export Audit Trail to CSV action
        self.export_audit_action = QAction("Export Audit Trail", self)
        self.export_audit_action.triggered.connect(self.export_audit_trail_to_csv)
        file_menu.addAction(self.export_audit_action)
        self.export_audit_action.setDisabled(True)
    ...
    def export_audit_trail_to_csv(self):
        try:
            # Select a file to save the CSV
            options = QFileDialog.Options()
            file_path, _ = QFileDialog.getSaveFileName(self, "Save Audit Trail to CSV", "", "CSV Files (*.csv);;All Files (*)", options=options)
            ...
            if file_path:
                ...
                QMessageBox.information(self, "Export Successful", f"Audit trail exported to {file_path}")
        except Exception as e:
            logging.error(f"Error exporting audit trail to CSV: {e}")
            QMessageBox.critical(self, "Export Error", f"An error occurred while exporting audit trail to CSV: {e}")
    ...
    def connecttosql(self, file_path):
        try:
            with sqlite3.connect(file_path) as conn:
                ...
                self.export_results_action.setDisabled(False)
    ...
    def display_image(self, index):
        ...
        if hasattr(self, 'processed_images'):
            ...
            if not self.current_image_index in self.processed_images.keys():
                ...
                self.processed_images[self.current_image_index] = {i: orig.astype(uint8) for i in range(4)}
